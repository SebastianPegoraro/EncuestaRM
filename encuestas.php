...
$stmt = $dbh->prepare("SELECT preguntas.encuesta_id,encuestas.titulo,count(*) as cantpreguntas FROM `preguntas` inner join encuestas on encuestas.id=preguntas.encuesta_id    group by preguntas.encuesta_id,encuestas.titulo");
  ...
  $stmt = $dbh->prepare("SELECT  elecciones.id,elecciones.descripcion,tipos.clase,tipos.id as idclase from opciones inner join elecciones on elecciones.id=opciones.eleccion_id inner join tipos on tipos.id=opciones.tipo_id where pregunta_id=".$idPregunta);
        $stmt->execute();
    $result = $stmt->fetchAll();
    if($stmt->rowCount()==0)
    {
